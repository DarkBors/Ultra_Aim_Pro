    ...
    for num_epochs in epochs:
        for test_size in test_sizes:
            for random_state in random_states:
                # Split the data
                X_train, X_test, y_train, y_test = train_test_split(X, Y_reliability, test_size=test_size, random_state=random_state)
...
X_train, X_test, y_train, y_test = train_test_split(X, Y_reliability, test_size=best_hyperparameters['test_size'], random_state=best_hyperparameters['random_state'])
model = keras.Sequential([
    keras.layers.Dense(32, activation='relu', input_shape=[X.shape[1]]),
    keras.layers.Dense(16, activation='relu'),
    keras.layers.Dense(1)  # One output: reliability
