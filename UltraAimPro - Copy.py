...
class GUI(tk.Tk):   
    def __init__(self):
        ...
        self.mainloop()
        #LOG
        logging.info("ℹ️ mainloop is started ")
    ...
    def open_data_file(self):
        ...
        if not file_path:
            #LOG
            logging.warning("⚠️ File could not be opened !")
            return  logging.warning("⚠️ File could not be opened !")
        ...
        with open(file_path, "r") as file:
            ...
            for line in file:
                row = line.split()
                row_data = {header[i]: float(value) for i, value in enumerate(row) if value.strip()}
